...
{
    public class ResourceFragment : MonoBehaviour, IInteractable
    {
        ...
        public InteractionType InteractionType => InteractionType.Fragment;

        public Vector3 GetInteractionPosition(Transform workerTransform) => transform.position;

        public bool CanInteract(Worker worker) => CanBePickedUp && !worker.IsInventoryFull;

        public void OnInteract(Worker worker) => worker.PickUpFragment(this, null);
        ...
        private void Awake()
        {
            // Start with Update disabled - only enable when launched
            enabled = false;
        }
        ...
        public void Launch()
        {
            ...
            // Enable Update for launch tracking
            enabled = true;

            Vector3 launchDir = CalculateLaunchDirection();
            rb.AddForce(launchDir * launchForce, ForceMode.Impulse);
            rb.AddTorque(0.3f * launchForce * UnityEngine.Random.insideUnitSphere, ForceMode.Impulse);
        }
        ...
        private void Update()
        {
            // Only runs when enabled (during launch)
            if (!isLaunched || isPickedUp || rb == null || rb.isKinematic) { enabled = false; return; }

            launchTimer += Time.deltaTime;

            // Check distance constraint using sqrMagnitude for performance
            float horizontalDistSqr =
                (transform.position.x - launchStartPosition.x) * (transform.position.x - launchStartPosition.x) +
                (transform.position.z - launchStartPosition.z) * (transform.position.z - launchStartPosition.z);

            // Stop if too far or time elapsed
            if (horizontalDistSqr > maxDistance * maxDistance || launchTimer >= settleTime) StopMovement();
        }
        ...
        private void StopMovement()
        {
            ...
            // Disable Update - no longer needed
            enabled = false;
        ...
        private bool IsSettled()
        {
            ...
            return rb.linearVelocity.sqrMagnitude < 0.01f;  // Use sqrMagnitude
        }
        ...
        public void PickUp(Transform carryPoint, int stackIndex)
        {
            ...
            // Disable Update if it was still running
            enabled = false;
        ...
        public void Drop(Action onDropComplete = null)
        {
            ...
            // If this component or its GameObject is inactive we cannot start coroutines.
            // In that case skip the animation, call completion and destroy immediately.
