...
namespace Assets.Scripts.Generation.Resources
{
    ...
    public class WoodInstructions : ResourceGenerationInstruction
    {
        ...
        {
            if (Config == null) { DebugManager.LogWarning("WoodInstructions: No config assigned!"); return null; }

            var spawnEntry = Config.GetRandomEntry();
            if (spawnEntry?.definition == null) { DebugManager.LogWarning("WoodInstructions: No valid resource definitions in config!"); return null; }

            // Ensure we have a tree definition
            var treeDefinition = spawnEntry.definition as TreeResourceDefinitionSO;
            if (treeDefinition == null) { DebugManager.LogWarning($"WoodInstructions: Definition '{spawnEntry.definition.resourceName}' is not a TreeResourceDefinitionSO!"); return null; }

            Vector3 spawnPos = SnapToGround(position);
            Resource tree = AssembleTreeResource(spawnPos, treeDefinition);

            if (tree != null) { GeneratedResources.Add(tree); DebugManager.LogSpawning($"WoodInstructions: Generated {treeDefinition.resourceName} (variant: {treeDefinition.VariantId}) at {spawnPos}"); }
        ...
        private Resource AssembleTreeResource(Vector3 position, TreeResourceDefinitionSO definition)
        {
            ...
            // Set the tree object to interactable layer (using base class method)
            SetInteractableLayer(treeObj);
            SetResourceTag(treeObj);
        ...
        private void BuildTreeVisuals(GameObject stageVisual, TreeResourceDefinitionSO definition)
        {
            ...
            for (int i = 0; i < totalPieces; i++)
            {
                ...
                if (prefab == null) { DebugManager.LogWarning($"WoodInstructions: No prefab available for piece {i}"); continue; }

                GameObject pieceObj = Instantiate(prefab, stageVisual.transform);
                pieceObj.transform.SetLocalPositionAndRotation(new Vector3(0f, currentHeight, 0f), Quaternion.identity);
