...
namespace Assets.Scripts.Resources
{
    public class Resource : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                if (resourceVisual == null && transform.childCount > 0) resourceVisual = transform.GetChild(0).gameObject;
        ...
        private void RefreshAvailablePieces()
        {
            ...
            if (resourceVisual == null) { DebugManager.LogWarning("Resource: resourceVisual is null, cannot find pieces!"); return; }

            var pieces = resourceVisual.GetComponentsInChildren<ResourcePiece>();

            foreach (var piece in pieces)
                if (!piece.IsDetached) availablePieces.Add(piece);
        ...
        {
            if (IsDepleted || CurrentStage == null) { onFragmentReady?.Invoke(null); return; }
        ...
        private void NotifyWaitingWorkers()
        {
            ...
            foreach (var worker in workersToNotify) StartWorking(worker.Callback, worker.WorkerTransform);
        }
        ...
        private IEnumerator WorkRoutine(System.Action<ResourceFragment> onFragmentReady, Transform workerTransform, object workerKey)
        {
            ...
            if (CurrentStage.yieldsPieces && availablePieces.Count > 0)
            {
                ...
                if (!skipWorkTime) yield return new WaitForSeconds(CurrentStage.workTimePerPiece);
                else { DebugManager.LogGathering("Resource: Instant pickup for last piece!"); yield return null; }
                ...
                if (availablePieces.Count > 0)
                {
                    ...
                    if (availablePieces.Count == 0) yield return StartCoroutine(TransitionToNextStage());
                }
                else DebugManager.LogGathering("Resource: No pieces available (taken by another worker)");
            }
            else
            {
                // Non-yielding stage
        ...
        private IEnumerator TransitionToNextStage()
        {
            ...
            if (currentStageIndex < stages.Length) InitializeStage(currentStageIndex);
            else
            {
