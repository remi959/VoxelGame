...
public class ResourcePointGeneratorEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Generate All Test Resources")) generator.GenerateAllTestResources();
        
        if (GUILayout.Button("Clear All Resources")) generator.ClearAllResources();
        ...
        if (generator.Instructions != null && generator.Instructions.Count > 0)
        {
            ...
            if (GUILayout.Button("Spawn At Position"))
            {
                ...
                if (instruction != null) instruction.GenerateResourceAt(manualSpawnPosition);
            }
        }
        ...
        if (GUILayout.Button("Create Wood Instruction")) CreateInstruction<WoodInstructions>("WoodInstruction");
        if (GUILayout.Button("Create Ore Instruction")) CreateInstruction<OreInstructions>("OreInstruction");
    ...
    private void CreateInstruction<T>(string defaultName) where T : ResourceGenerationInstruction
    {
        ...
        {
            var instruction = CreateInstance<T>();
