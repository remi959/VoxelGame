...
namespace Assets.Scripts.Generation.Resources
{
    ...
    public class OreInstructions : ResourceGenerationInstruction
    {
        ...
        [SerializeField] private float harvestTimePerPiece = 1.5f;
        ...
        public override Resource GenerateResourceAt(Vector3 position)
        {
            if (Config == null)
            {
                DebugManager.LogWarning("OreInstructions: No config assigned!");
                return null;
            }
            ...
            {
                DebugManager.LogWarning("OreInstructions: No valid resource definitions in config!");
                return null;
            }

            Vector3 spawnPos = SnapToGround(position);
            Resource vein = AssembleOreVein(spawnPos, spawnEntry);

            if (vein != null)
            {
                GeneratedResources.Add(vein);
                DebugManager.LogSpawning($"OreInstructions: Generated {spawnEntry.definition.resourceName} vein at {spawnPos}");
            }
        ...
        private Resource AssembleOreVein(Vector3 position, ResourceGenerationConfig.ResourceSpawnEntry entry)
        {
            var definition = entry.definition;

            // Create parent object
            GameObject veinObj = new GameObject($"Vein_{definition.resourceName}");
            ...
            BuildVeinVisuals(veinVisual, definition);

            // Add Resource component and initialize as simple resource
            var resource = veinObj.AddComponent<Resource>();

            resource.InitializeSimple(
                type: definition.resourceType,
                visual: veinVisual,
                workTimePerPiece: harvestTimePerPiece,
                destroyOnComplete: true,
                order: EHarvestOrder.Closest
            );
        ...
        private void BuildVeinVisuals(GameObject veinVisual, ResourceDefinitionSO definition)
        {
            int pieceCount = Random.Range(definition.minTrunkPieces, definition.maxTrunkPieces + 1);
            ...
            {
                // Get prefab - use trunk prefabs array for ore pieces
                GameObject prefab = definition.GetRandomTrunkPrefab();

                if (prefab == null)
                {
                    DebugManager.LogWarning($"OreInstructions: No prefab available for piece {i}");
                    continue;
                }
                ...
                if (!pieceObj.TryGetComponent<ResourcePiece>(out var piece))
                {
        ...
        private Vector3[] GenerateVeinPositions(int count)
        {
            ...
            {
                if (i < BaseLayerPattern.Length)
                {
                    // Use base layer pattern
                    positions[i] = BaseLayerPattern[i];
                }
                else
                {
                    // Use upper layer pattern
                    int upperIndex = i - BaseLayerPattern.Length;
                    if (upperIndex < UpperLayerPattern.Length)
                    {
                        positions[i] = UpperLayerPattern[upperIndex];
                    }
                    else
                    {
        ...
        private void PreviewVeinPattern()
        {
            ...
            {
                Debug.Log($"Piece {i + 1}: {positions[i]}");
            }
        }
