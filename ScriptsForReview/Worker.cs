...
namespace Assets.Scripts.NPCs.Units
{
    public class Worker : NPCBase
    {
        ...
        protected override void OnDisable()
        {
            ...
            // Return all fragments to pool when disabled
            ForceReturnAllFragments();
        }

        private void OnDestroy()
        {
            // Ensure fragments are cleaned up when worker is destroyed
            ForceReturnAllFragments();
        }
        ...
        public override void InteractWith(GameObject target)
        {
            ...
            if (target.TryGetComponent<StoragePoint>(out var storage))
            {
                if (carriedAmount > 0)
                {
                    if (storage.AcceptsType(carriedType))
                    {
                        DepositAt(storage);
                        return;
                    }
                    ...
                    {
                        DebugManager.LogInventory($"Worker: Storage doesn't accept {carriedType}");
                    }
                }
                else
                {
                    DebugManager.LogInventory("Worker: Nothing to deposit");
                }
                return;
            }
        ...
        {
            if (storage == null || carriedAmount <= 0)
            {
        ...
        public void AddToInventory(ResourceFragment fragment)
        {
            if (fragment == null)
            {
                DebugManager.LogWarning("Worker: Tried to add null fragment to inventory");
                return;
            }
        ...
        public void ClearInventory()
        {
            ...
            {
                if (fragment != null && !fragment.IsDropping)
                {
                    fragment.Drop();
                }
            }
        ...
        /// <summary>
        /// Immediately return all fragments to pool (for worker destruction/disable).
        /// </summary>
        public void ForceReturnAllFragments()
        {
            foreach (var fragment in carriedFragments)
            {
                if (fragment != null)
                {
                    fragment.ReturnToPoolImmediate();
                }
            }
        ...
        private void ReindexFragments()
        {
            for (int i = 0; i < carriedFragments.Count; i++)
            {
                ...
                if (fragment != null)
                {
                    fragment.UpdateStackPosition(i);
                }
            }
        }
